...
class Dispatcher:
    ...
    职责:
    1. 解析 TC-IR 的执行模式
    2. 将步骤路由到对应引擎 (UI → Midscene, API → 左瞳)
    3. 管理执行上下文
    4. 收集执行轨迹
    """
    ...
    async def _execute_step(self, step: dict, mode: ExecutionMode, context: Optional[Dict[str, Any]] = None, downstream_var_names: Optional[List[str]] = None, execution_id: Optional[str] = None) -> dict:
        ...
        if engine_type == "MIDSCENE":
            raise RuntimeError("UI steps must be executed through MidsceneAdapter before Dispatcher")

        if engine_type == "LEFT_PUPIL":
