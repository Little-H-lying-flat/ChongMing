...
# Singleton instance of TurboEngine
# In a real production app, this might be managed by dependency injection
...
async def start_turbo_test(config: TurboRunConfig, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        # Let's await it.
        test_id = await turbo_engine.run_test(config)
...
def stop_turbo_test(test_id: str):
    ...
    """
    turbo_engine.stop_test(test_id)
...
def get_turbo_stats(test_id: str):
    ...
    """
    stats = turbo_engine.get_stats(test_id)
    if not stats:
